...
//  Edited by anna.zubakina on 23/11/2023
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let navController = tabBarController?.viewControllers?.first as? UINavigationController {
            if let homeViewController = navController.topViewController as? HomeViewController {
                homeViewController.performSegue(withIdentifier: "detailMovie", sender: selectedMovie)
            }
        }
    }

    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "detailMovie", let destinationVC = segue.destination as? DetailViewController {
            if let selectedMovie = sender as? Movie {
                destinationVC.movies = selectedMovie
            }
        }
    }
